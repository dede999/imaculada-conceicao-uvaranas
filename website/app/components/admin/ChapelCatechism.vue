...
const form   = reactive({ group_name: '', day_of_week: '', time: '' })

async function add() {
  if (!form.group_name) { emit('error', 'Nome do grupo obrigatório.'); return }
  saving.value = true
  try {
    await $fetch('/api/admin/catechism', {
      method: 'POST',
      body: {
        chapel_id: props.chapelId,
    ...
    open.value = false
    form.group_name = ''; form.day_of_week = ''; form.time = ''
    emit('added')
  }
  catch (e: unknown) {
...
<template>
  <div class="schedule-block schedule-block--last">
    ...
    <div v-if="groups.length" class="rows-list">
      <div v-for="g in groups" :key="g.id" class="schedule-row">
        ...
        <button class="btn-delete" :disabled="deleting === g.id" @click="emit('delete', g.id)">✕</button>
      </div>
    </div>
    <p v-else class="empty-hint">Nenhuma turma cadastrada.</p>

    <form v-if="open" class="inline-form" @submit.prevent="add">
      <input v-model="form.group_name" type="text" class="form-input form-input--wide" placeholder="Nome do grupo (ex: Primeira Eucaristia)" required />
...
.schedule-block--last { border-bottom: none; }
