...
const form   = reactive({ day_of_week: 0, time_start: '', time_end: '' })

async function add() {
  if (!form.time_start || !form.time_end) { emit('error', 'Horários obrigatórios.'); return }
  saving.value = true
  try {
    await $fetch('/api/admin/confessions', {
      method: 'POST',
      body: { chapel_id: props.chapelId, day_of_week: form.day_of_week, time_start: form.time_start, time_end: form.time_end },
    })
    open.value = false
    form.day_of_week = 0; form.time_start = ''; form.time_end = ''
    emit('added')
  }
  catch (e: unknown) {
...
<template>
  <div class="schedule-block">
    ...
    <div v-if="confessions.length" class="rows-list">
      <div v-for="c in confessions" :key="c.id" class="schedule-row">
    ...
    <form v-if="open" class="inline-form" @submit.prevent="add">
      ...
      <input v-model="form.time_start" type="time" class="form-input" required />
      <span class="form-sep">–</span>
      <input v-model="form.time_end" type="time" class="form-input" required />
...
.form-sep { font-family: var(--font-sans); font-size: 14px; color: var(--text-muted); }
