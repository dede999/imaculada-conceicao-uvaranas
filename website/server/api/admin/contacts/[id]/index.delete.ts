...
export default defineEventHandler(async (event) => {
  ...
  const { data: rec } = await supabase.from('chapel_contacts').select('chapel_id').eq('id', id as never).maybeSingle()
  ...
  const { error } = await supabase.from('chapel_contacts').delete().eq('id', id as never)
