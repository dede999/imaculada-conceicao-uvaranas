...
export default defineEventHandler(async (event) => {
  ...
  const { data: rec } = await supabase.from('chapel_images').select('chapel_id').eq('id', id as never).maybeSingle()
  ...
  const { error } = await supabase.from('chapel_images').delete().eq('id', id as never)
